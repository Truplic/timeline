...
export const timelinePlugin = (opts) => {
  const { mode, count, fill, stroke, stateLabels } = opts;

  const laneWidth = 0.9;
  const laneDistr = SPACE_BETWEEN;

  const walk = (yIdx, count, dim, draw) => {
      ...
      (
        ...
        for (let ix = 0; ix < dataY.length; ix++) {
          if (dataY[ix] != null) {
            ...
            u.ctx.fillText(stateLabels[dataY[ix]], x, y);
          }
        }
      },
    );
      ...
      ready: [
        (u) => {
          u.over.addEventListener("click", (e) => {
            ...
            let found = null;
            // Query the quadtree for a 1x1 pixel area under the mouse
            qt.get(cx, cy, 1, 1, (o) => {
            ...
            if (found) {
              alert(
                `Hit Block! Sigout: ${found.sidx}, Waveform: ${u.data[found.sidx][found.didx]}`,
              );
            }
          });
        },
